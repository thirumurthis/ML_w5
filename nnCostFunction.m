...
%for i= 1:num_labels,
%	h1=sigmoid(X * Theta1);
%	y * log(h1) + (1 - y) * log(1-h1);
%end

z2= A1x * Theta1';
%activation of layer 2 - hidden_layer
A2 = [ones(size(z2,1),1) sigmoid(z2)];

z3 = A2 * Theta2';
%activation of layer 3 -output layer
Htheta=A3 = sigmoid(z3);

%regression attribute
%reg = lambda/2*m * (sum(sum(Theta1(:,2:end).^2,2)) + sum(sum(Theta2(:,2:end).^2,2)));
reg = lambda/(2*m) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));

%J = (1/m)*sum(sum((-Y).*log(Htheta) - (1-Y).*log(1-Htheta), 2));
...
%Theta1_grad = sum(A1x * d2);
%Theta2_grad = sum(A2 * d3);
%d1 = ((Theta1)' * d2).*A1x .*(1-A1x);
...
%J1 = (1/m)*(-y' * log(h1) - (1-y)' * log (1-h1));

%h2=sigmoid(h1 * Theta2);

%J2 = (1/m) * (-y' * log(h2) - (1-y)' * log (1-h2));
